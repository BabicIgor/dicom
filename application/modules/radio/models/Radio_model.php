...
class Radio_model extends CI_Model{
    ...
    public function get_radios_by_row($row_id)
    {
    ...
    public function delete_radio_by_rowid($row_id)
    {
    ...
    public function get_radiologists($institution_id=null)
    {
        
        if($institution_id == null)
        {
            
            $query = $this->db->query('SELECT a.*, b.*, c.*, c.`email` as inst_email, a.`email` as phy_email FROM `physician` a LEFT JOIN `address` b ON a.`address` = b.`address_id` LEFT JOIN `institution` c ON a.`institution` = c.`institution_id` WHERE a.`physician_type` = "radio"');
        ...
        else
        {
            ...
            $query = $this->db->query('SELECT a.*, b.*, c.*, c.`email` as inst_email, a.`email` as phy_email FROM `physician` a LEFT JOIN `address` b ON a.`address` = b.`address_id` LEFT JOIN `institution` c ON a.`institution` = c.`institution_id` WHERE c.`institution_id` = '.$institution_id.' AND a.`physician_type` = "radio"');
            $query_result = $query->result();
            if (sizeof($query_result) > 0) {
                return $query->result();
            } else {
    ...
    public function update_radio($param_data)
    {
    ...
    public function new_radiologist($param_data){
