...
angular.module('MetronicApp').controller('FacilityListController', ['$rootScope', '$scope', 'settings', '$state', 'DTOptionsBuilder', 'DTColumnDefBuilder', '$http', 'toastr', function($rootScope, $scope, settings, $state, DTOptionsBuilder, DTColumnDefBuilder, $http, toastr) {
	...
	setColumnActive = function(idx)
	{
	    ...
	    $scope.update_facility = function(row_id)
	    {
	        $state.go('facility.update', {row_id : row_id});
	    }




	    $scope.add_doctor_by_facility = function(row_id)
	    {
	        $state.go('facility.addDoctor', {row_id : row_id});
	    }
	    $scope.add_radio_by_facility = function(row_id)
	    {
	        $state.go('facility.addRadio', {row_id : row_id});
	    }
	    $scope.add_patient_by_facility = function(row_id)
	    {
	        $state.go('facility.addPatient', {row_id : row_id});
	    }
	   ...
	   // }
	    ...
	    $scope.delete_institution_confirm = function(row_id)
	    {
    		...
    		.success(function (responseData,status,headers,config) {
                    ...
                    $state.transitionTo('facility.list', {}, {reload:true});
