...
angular.module('MetronicApp').controller('DoctorListController', ['$rootScope', '$scope', 'settings', '$state', 'DTOptionsBuilder', 'DTColumnDefBuilder', '$http', 'toastr', function($rootScope, $scope, settings, $state, DTOptionsBuilder, DTColumnDefBuilder, $http, toastr) {
	...
	$scope.row_edit = function(row_id){
	    $state.go('doctor.edit', {row_id : row_id});
	}
	...
	$scope.delete_confirm = function(row_id)
	{
        ...
        $(html).modal('hide');
	    $http.get(base_url + 'doctor/deleteDoctorById?row_id=' + row_id)
    	.success(function (responseData,status,headers,config) {
    	    toastr.success('Deleted Successfully');
            $state.transitionTo('doctor.list', {}, {reload:true});
    	})
    	.error(function(data){
    	});	
	}
