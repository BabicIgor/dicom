...
angular.module('MetronicApp').controller('HospitalListController', ['$rootScope', '$scope', 'settings', '$state', 'DTOptionsBuilder', 'DTColumnDefBuilder', '$http', 'toastr', function($rootScope, $scope, settings, $state, DTOptionsBuilder, DTColumnDefBuilder, $http, toastr) {
    $scope.userType = $rootScope.userRole;
	...
	setColumnActive = function(idx)
	{
		...
		$scope.records = [];
	    $http.get(base_url + 'hospital/getInstitution')
		.success(function (responseData,status,headers,config) {
			
			
			$scope.records = responseData;
			
			for(var idx = 0;idx < $scope.records.length; idx++)
			{
    	    	var state_arr = s_a[parseInt($scope.records[idx].country) + 1].split("|");

			    $scope.records[idx].country = country_arr[$scope.records[idx].country];
			    $scope.records[idx].state = state_arr[$scope.records[idx].state];
			}
			setTimeout(function(){
	    ...
	    $scope.add_doctor_by_institution = function(row_id)
	    {
	        $state.go('hospital.addDoctor', {row_id : row_id});
	    }
	    $scope.add_radio_by_institution = function(row_id)
	    {
	        $state.go('hospital.addRadio', {row_id : row_id});
	    }
	    $scope.add_patient_by_institution = function(row_id)
	    {
	        $state.go('hospital.addPatient', {row_id : row_id});
	    }
	    $scope.update_institution = function(row_id)
	    {
	        $state.go('hospital.update', {row_id : row_id});
	    }
	    
	    $scope.add_facility_by_institution = function(row_id)
	    {
	        $state.go('hospital.addFacility', {row_id : row_id});
	    }
	    $scope.add_radiology_by_institution = function(row_id)
	    {
   	        $state.go('hospital.addRadiology', {row_id : row_id});
	    }
	    ...
	    $scope.delete_institution_confirm = function(row_id)
	    {
	     
	        var html = '#myModal_autocomplete_' + row_id;
             $(html).modal('hide');

	        $http.get(base_url + 'hospital/deleteInstitution?row_id=' + row_id)
    		.success(function (responseData,status,headers,config) {
    			
    			
		    	    toastr.success('Deleted Successfully');
                    $state.transitionTo('hospital.list', {}, {reload:true});
