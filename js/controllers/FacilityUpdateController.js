...
angular.module('MetronicApp').controller('FacilityUpdateController', ['$rootScope', '$scope', 'settings', '$state', '$http', '$stateParams', function($rootScope, $scope, settings, $state, $http, $stateParams) {
    $scope.row = [];
    ...
    $scope.facility_update = function()
    {
        ...
        !$scope.zipcodevalidation)
        {
            ...
            $http.post(base_url + 'hospital/updateFacilityByInstAdmin', data)
    		.success(function (responseData,status,headers,config) {
    ...
    $scope.close_update_facility = function()
    {
        $state.go('facility.list');
    }
